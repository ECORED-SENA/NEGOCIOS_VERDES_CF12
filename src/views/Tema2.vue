<template lang="pug">
.curso-main-container.pb-3
  BannerInterno
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5.mb-5  
    .titulo-principal(data-aos="fade-left")
      .titulo-principal__numero
        span 2
      h1 Campaña promocional

    p(data-aos="fade-down") Se puede definir como un proceso estratégico de gran importancia para posicionar el producto en el mercado y mezclándose con estrategias adecuadas de #[i marketing]. Además, se logrará la inclinación de compra hacia el producto, así como la identificación de marca.

    .row.justify-content-between.align-items-center.mt-5
      .col-lg-2(data-aos="fade-left")
        img.img-t.yoyo1(src='@/assets/ilustraciones/img19.svg', alt='')
        h5.mt-3 Campaña publicitaria
        .hr
      .col-lg-9.col-color-1.white-border(data-aos="fade-right")
        p.p-5.mb-0 Es un amplio plan de publicidad para una serie de anuncios diferentes, pero relacionados, que aparecen en diversos medios durante un periodo específico. La campaña está diseñada en forma estratégica para conseguir cumplir un grupo de objetivos y resolver algún problema crucial; además, se trata de un plan a corto plazo que, por lo general, funciona durante un año o menos. 
      
    .row.justify-content-start.align-items-center.mt-5
      .col-lg-8(data-aos="fade-right")
        p.mb-0 De otra parte, en un plan de campaña se resume la situación en el mercado y las estrategias y tácticas para las áreas primarias de creatividad y medios, así como otras áreas de comunicación, de mercadotecnia, de promoción, de ventas, mercadotecnia directa y relaciones públicas; este plan de campaña se debe exponer al cliente en una presentación de negocios formal (Elisea, 2003).
      .col-lg-3.my-3(data-aos="fade-left")      
        img.img-.yoyo1(src='@/assets/ilustraciones/img20.svg', alt='')

    p(data-aos="fade-down").mt-5 #[b Con respecto a lo anterior, se puede indicar que es un libro de planes donde se resumen los objetivos, campañas de mercado, estrategias y promoción de ventas.]
    h4(data-aos="fade-down").mt-5 Elementos de la campaña publicitaria 
    .row.justify-content-center.align-items-center.row-bg-color-1.p-5.mt-5(data-aos="fade-left")
      .col-lg-12.mb-5.pb-4.d-none.d-lg-block 
        p.text-center(data-aos="fade-up") #[b A continuación, se conocerán los elementos que debe tener una campaña publicitaria: ]
        ImagenInfografica.color-secundario.mt-5
          template(v-slot:imagen)
            figure
              img.mb-5(data-aos="zoom-in")(src='@/assets/ilustraciones/img21.svg', alt='')

          .tarjeta.color-acento-botones.p-3(x="6.6%" y="36%")
            .h5.mb-2 Descripción del producto o servicio
            p Es donde definimos qué se va a comercializar. Debemos hacer un historial detallado teniendo en cuenta el comportamiento del mercado.

          .tarjeta.color-acento-botones.p-3(x="17.1%" y="51%")
            .h5.mb-2 Establecer objetivos
            p Como ya sabemos, nuestra finalidad es que nos conozcan, empiecen a consumir el producto e incrementar las ventas. Pero como todo no pueden ser ventas, tendremos en cuenta otras variables a la hora de trazar los objetivos, debemos ser tácticos para llamar la atención, informar y recordar; y estratégicos para hacer que la frecuencia de la compra aumente y sea rentable.

          .tarjeta.color-acento-botones.p-3(x="28.9%" y="62.1%")
            .h5.mb-2 Definición del público objetivo
            p Es fundamental definir a quién nos vamos a dirigir, debemos ser cuidadosos al escoger el tipo de personas que vamos a impactar teniendo en cuenta variables como el sexo, nivel de ingreso, condiciones sociales, etc.   

          .tarjeta.color-acento-botones.p-3(x="40.7%" y="69.7%")
            .h5.mb-2 Determinación del ámbito geográfico
            p Dimensionar el ámbito de la campaña, los cuales pueden ser locales, regionales, nacionales e internacionales. 

          .tarjeta.color-acento-botones.p-3(x="54.3%" y="67.5%")
            .h5.mb-2 Diseño del mensaje
            p En este punto debemos tener en cuenta el contenido, ya que puede ser informativo, moral, emocional, etc., contener una base de argumentos sólidos y llevar mucha creatividad. 

          .tarjeta.color-acento-botones.p-3(x="65.66%" y="61.5%")
            .h5.mb-2 Plan de medios
            p Es hora de definir qué medios vamos a usar para difundir el mensaje, existen diferentes medios de publicidad dentro de los que encontramos, redes sociales, publicidad móvil, publicidad con video, publicidad impresa, publicidad por radio, publicidad por teléfonos, etc. 

          .tarjeta.color-acento-botones.p-3(x="77%" y="51.8%")
            .h5.mb-2 Presupuesto
            p Es indispensable saber el recurso humano y económico para poder ejecutarlo. 

          .tarjeta.color-acento-botones.p-3(x="87.7%" y="39.4%")
            .h5.mb-2 Medición de la eficacia publicitaria
            p Aquí debemos analizar que los objetivos trazados anteriormente obtengan los resultados esperados.
      .col-lg-12.d-block.d-lg-none
        p.text-center.mb-4(data-aos="fade-up") #[b A continuación, se conocerán los elementos que debe tener una campaña publicitaria: ]      
        .row.justify-content-center.align-items-center(data-aos="fade-left")
          .col-3.d-block.d-lg-none.px-0.mx-0
            img.img-t.img-a.beat(src='@/assets/ilustraciones/img91.svg', alt='')
          .col-9.d-block.d-lg-none.info-movil-bg-2.p-3.mb-0
            .h5.mb-2 Descripción del producto o servicio
            p.mb-0 Es donde definimos qué se va a comercializar. Debemos hacer un historial detallado teniendo en cuenta el comportamiento del mercado.   

        .row.justify-content-center.align-items-center.mt-4(data-aos="fade-right")
          .col-3.d-block.d-lg-none.px-0.mx-0
            img.img-t.img-a.beat(src='@/assets/ilustraciones/img92.svg', alt='')
          .col-9.d-block.d-lg-none.info-movil-bg-2.p-3.mb-0
            .h5.mb-2 Establecer objetivos
            p.mb-0 Como ya sabemos, nuestra finalidad es que nos conozcan, empiecen a consumir el producto e incrementar las ventas. Pero como todo no pueden ser ventas, tendremos en cuenta otras variables a la hora de trazar los objetivos, debemos ser tácticos para llamar la atención, informar y recordar; y estratégicos para hacer que la frecuencia de la compra aumente y sea rentable. 

        .row.justify-content-center.align-items-center.mt-4(data-aos="fade-left")
          .col-3.d-block.d-lg-none.px-0.mx-0
            img.img-t.img-a.beat(src='@/assets/ilustraciones/img93.svg', alt='')
          .col-9.d-block.d-lg-none.info-movil-bg-2.p-3.mb-0
            .h5.mb-2 Definición del público objetivo
            p.mb-0 Es fundamental definir a quién nos vamos a dirigir, debemos ser cuidadosos al escoger el tipo de personas que vamos a impactar teniendo en cuenta variables como el sexo, nivel de ingreso, condiciones sociales, etc.  

        .row.justify-content-center.align-items-center.mt-4(data-aos="fade-right")
          .col-3.d-block.d-lg-none.px-0.mx-0
            img.img-t.img-a.beat(src='@/assets/ilustraciones/img94.svg', alt='')
          .col-9.d-block.d-lg-none.info-movil-bg-2.p-3.mb-0
            .h5.mb-2 Determinación del ámbito geográfico
            p.mb-0 Dimensionar el ámbito de la campaña, los cuales pueden ser locales, regionales, nacionales e internacionales.  

        .row.justify-content-center.align-items-center.mt-4(data-aos="fade-left")
          .col-3.d-block.d-lg-none.px-0.mx-0
            img.img-t.img-a.beat(src='@/assets/ilustraciones/img95.svg', alt='')
          .col-9.d-block.d-lg-none.info-movil-bg-2.p-3.mb-0
            .h5.mb-2 Diseño del mensaje
            p.mb-0 En este punto debemos tener en cuenta el contenido, ya que puede ser informativo, moral, emocional, etc., contener una base de argumentos sólidos y llevar mucha creatividad.  

        .row.justify-content-center.align-items-center.mt-4(data-aos="fade-right")
          .col-3.d-block.d-lg-none.px-0.mx-0
            img.img-t.img-a.beat(src='@/assets/ilustraciones/img96.svg', alt='')
          .col-9.d-block.d-lg-none.info-movil-bg-2.p-3.mb-0
            .h5.mb-2 Plan de medios
            p.mb-0 Es hora de definir qué medios vamos a usar para difundir el mensaje, existen diferentes medios de publicidad dentro de los que encontramos, redes sociales, publicidad móvil, publicidad con video, publicidad impresa, publicidad por radio, publicidad por teléfonos, etc. 

        .row.justify-content-center.align-items-center.mt-4(data-aos="fade-left")
          .col-3.d-block.d-lg-none.px-0.mx-0
            img.img-t.img-a.beat(src='@/assets/ilustraciones/img97.svg', alt='')
          .col-9.d-block.d-lg-none.info-movil-bg-2.p-3.mb-0
            .h5.mb-2 Presupuesto
            p.mb-0 Es indispensable saber el recurso humano y económico para poder ejecutarlo.  

        .row.justify-content-center.align-items-center.mt-4(data-aos="fade-right")
          .col-3.d-block.d-lg-none.px-0.mx-0
            img.img-t.img-a.beat(src='@/assets/ilustraciones/img98.svg', alt='')
          .col-9.d-block.d-lg-none.info-movil-bg-2.p-3.mb-0
            .h5.mb-2 Medición de la eficacia publicitaria
            p.mb-0 Aquí debemos analizar que los objetivos trazados anteriormente obtengan los resultados esperados. 


    separador
    #t_2_1.titulo-segundo(data-aos="fade-right")
      h2 2.1 Tipos de campaña publicitaria     
    p(data-aos="fade-down") Las campañas publicitarias se clasifican en dos: #[b comerciales y no comerciales.]
    h4(data-aos="fade-right").mt-5 Comerciales
    p(data-aos="fade-down") En el siguiente video se puede descubrir en qué consisten los 
      span(style='background-color: #E5DEC1') comerciales: 
    figure.mt-5
      .video
        iframe(width="560" height="315" src="https://www.youtube.com/embed/lWUFSgl8DUU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)
      figcaption Video: CF012_2.1_Tipos de campaña publicitaria.
    h4(data-aos="fade-right").mt-5 No comerciales
    p(data-aos="fade-down") Este tipo de comerciales promueven ideas y/o personas.      
    .row.justify-content-center.align-items-stretch.mt-5
      .col-lg-5.col-color-3.white-border-1.p-0.mx-0(data-aos="fade-right")
        img.img-t.img-a(src='@/assets/ilustraciones/img22.svg', alt='')
        h5.px-5.mt-5 Propaganda: 
        p.px-5.pb-3 Es cuando no hay un interés económico de por medio, es decir, solo se hace para promover ideas, personas e ideologías.  

      .col-lg-5.col-color-3.white-border-1.p-0.mx-0(data-aos="fade-left")
        img.img-t.img-a(src='@/assets/ilustraciones/img23.svg', alt='')
        h5.px-5.mt-5 Cívica o de bien público: 
        p.px-5.pb-3 Son hechas por asociaciones, fundaciones y entidades al servicio de la comunidad, son sin ánimo de lucro y buscan  cambiar actitudes masivas con productos no rentables. 

    separador
    #t_2_2.titulo-segundo(data-aos="fade-right")
      h2 2.2 Estructura de una campaña publicitaria 

    p(data-aos="fade-down") Cuando se realiza una campaña publicitaria es de suma importancia 
      span(style='background-color: #E5DEC1') #[b estudiar con profundidad el producto o servicio base],
        span(style='background-color: white')  conociendo de fondo aspectos como: qué tanto reconocimiento tiene este en el mercado, si es necesario incluir novedades, analizar si se le da una identidad institucional o social. Teniendo en cuenta los aspectos anteriormente mencionados, es más fácil escoger el tipo de campaña adecuada para llegar al público meta. 
    h4.mt-5(data-aos="fade-down") ¿Cómo hacer una campaña promocional?
    .row.justify-content-end.align-items-center.row-bg-color-2.row-m-top.p-4(data-aos="flip-down") 
      .col-lg-7(data-aos="slide-right")
        p.px-5 Al planificar una campaña de promoción, hay que tener en cuenta que esta se compone de tres resultados deseados y de varios pasos para una campaña promocional, a saber:
        ol.lista-ol--cuadro.px-5
          li 
            .lista-ol--cuadro__vineta
              span 1
            p #[b El mensaje de promoción llega a su público objetivo y la intención.]
          li 
            .lista-ol--cuadro__vineta
              span 2
            p #[b Su mensaje es entendido por su público.]
          li 
            .lista-ol--cuadro__vineta
              span 3
            p #[b Su mensaje estimula a los receptores para tomar las acciones oportunas (Ortego, 2021).]

    .row.justify-content-start.align-items-center(data-aos="flip-down") 
      .col-lg-5(data-aos="slide-left").d-none.d-lg-block
          img.img-t.img-a.img-flot.yoyo(src='@/assets/ilustraciones/img24.svg', alt='')       

    p(data-aos="fade-down").mt-5 #[b Para que la planificación de la campaña sea exitosa, es necesario tener en cuenta los siguientes parámetros:]

    .row.justify-content-start.align-items-center.row-bg-color-3.row-m-top.p-5.bg-img-1(data-aos="slide-left")
      .col-lg-12
        img.img-t.img-a.img-flot-1.yoyo1(src='@/assets/ilustraciones/img25.svg', alt='')(data-aos="slide-up") 
        .row.justify-content-center.align-items-center.mt-5(data-aos="zoom-in-left")
          .col-lg-5.d-none.d-lg-block
            img.img-t.img-a.yoyo(src='@/assets/ilustraciones/img26.svg', alt='') 
          .col-lg-5.d-none.d-lg-block
            .row.justify-content-center.align-items-center
              .col-lg-12.p-color-3.p-3
                img.img-t.num-top(src='@/assets/ilustraciones/1.svg', alt='')
                h5.mt-3 Definir objetivos
                p Lo primero que debemos hacer es definir los objetivos que deben ser claros y fáciles de alcanzar, es necesario iniciar la comunicación con el público objetivo. 

          .col-lg-5.d-block.d-lg-none
            .row.justify-content-center.align-items-center.mt-5
              .col-lg-12.p-color-7.p-3
                img.img-t.num-top(src='@/assets/ilustraciones/1.svg', alt='')
                h5.mt-3 Definir objetivos
                p Lo primero que debemos hacer es definir los objetivos que deben ser claros y fáciles de alcanzar, es necesario iniciar la comunicación con el público objetivo.                  
      
        .row.justify-content-center.align-items-center.mt-5(data-aos="zoom-in-right")
          .col-lg-5.d-none.d-lg-block
            .row.justify-content-center.align-items-center
              .col-lg-12.p-color-4.p-3
                img.img-t.num-top(src='@/assets/ilustraciones/2.svg', alt='')
                h5.mt-3 Definir el público objetivo
                p Es de vital importancia saber qué público se quiere alcanzar para realizar el contenido del mensaje y  saber qué medio de comunicación es el mejor, debemos usar herramientas como investigaciones de mercado y mejorar los medios comunicativos.     

          .col-lg-5.d-block.d-lg-none
            .row.justify-content-center.align-items-center.mt-5
              .col-lg-12.p-color-7.p-3
                img.img-t.num-top(src='@/assets/ilustraciones/2.svg', alt='')
                h5.mt-3 Definir el público objetivo
                p Es de vital importancia saber qué público se quiere alcanzar para realizar el contenido del mensaje y  saber qué medio de comunicación es el mejor, debemos usar herramientas como investigaciones de mercado y mejorar los medios comunicativos.                      
          .col-lg-5.d-none.d-lg-block
            img.img-t.img-a.yoyo(src='@/assets/ilustraciones/img27.svg', alt='')

        .row.justify-content-center.align-items-center.mt-5(data-aos="zoom-in-left")
          .col-lg-5.d-none.d-lg-block
            img.img-t.img-a.yoyo(src='@/assets/ilustraciones/img28.svg', alt='') 
          .col-lg-5.d-none.d-lg-block
            .row.justify-content-center.align-items-center
              .col-lg-12.p-color-5.p-3
                img.img-t.num-top(src='@/assets/ilustraciones/3.svg', alt='')
                h5.mt-3 Planeación
                p En esta etapa vamos a organizar las estrategias partiendo de la información en los ítems anteriores, analizaremos fecha de estrenos, periodo de presentación, entre otras. 

          .col-lg-5.d-block.d-lg-none
            .row.justify-content-center.align-items-center.mt-5
              .col-lg-12.p-color-7.p-3
                img.img-t.num-top(src='@/assets/ilustraciones/3.svg', alt='')
                h5.mt-3 Planeación
                p En esta etapa vamos a organizar las estrategias partiendo de la información en los ítems anteriores, analizaremos fecha de estrenos, periodo de presentación, entre otras. 

        .row.justify-content-center.align-items-center.mt-5(data-aos="zoom-in-right")
          .col-lg-5.d-none.d-lg-block
            .row.justify-content-center.align-items-center
              .col-lg-12.p-color-6.p-3
                img.img-t.num-top(src='@/assets/ilustraciones/4.svg', alt='')
                h5.mt-3 Creación del #[i briefing]
                p El paso final es la creación del documento que contiene información puntual del producto, población meta, posicionamiento de la marca, distribución geográfica, logística, actividades promocionales y presupuesto. 

          .col-lg-5.d-block.d-lg-none
            .row.justify-content-center.align-items-center.mt-5
              .col-lg-12.p-color-7.p-3
                img.img-t.num-top(src='@/assets/ilustraciones/4.svg', alt='')
                h5.mt-3 Creación del #[i briefing]
                p El paso final es la creación del documento que contiene información puntual del producto, población meta, posicionamiento de la marca, distribución geográfica, logística, actividades promocionales y presupuesto.                            
          .col-lg-5.d-none.d-lg-block
            img.img-t.img-a.yoyo(src='@/assets/ilustraciones/img29.svg', alt='')                

    separador
    #t_2_3.titulo-segundo(data-aos="fade-right")
      h2 2.3 #[i Brochure] 
    p(data-aos="fade-down") Es un folleto que contiene la oferta y servicios que ofrece la empresa. Está catalogado como una potente herramienta del #[i marketing] versátil que se adapta muy bien a las necesidades del negocio, pues es una forma fácil de llegar a los clientes o posibles clientes.

    .row.justify-content-center.align-items-center.mt-5(data-aos="slide-left")
      .col-lg-5.p-0
        img.img-a(src='@/assets/ilustraciones/img31.svg', alt='') 
      .col-lg-7.col-color-4.p-5
        p.mb-0 Para que el #[i brochure] logre la efectividad que se desea, es necesario incluir información relevante para los clientes, imágenes que resalten datos principales, usar letra especializada que llame la atención del consumidor, así como la forma de distribuir la información de tal manera que no falten datos, además de los datos del contacto de la empresa.

    h4(data-aos="fade-right").mt-5 Tipos de #[i brochure] 
    p(data-aos="fade-down") Hay diferentes clases de #[i brochure], estos son: 

    .row.justify-content-center.align-items-stretch.mt-5
      .col-lg-4.col-color-3.white-border-1.p-0.mx-0(data-aos="fade-right")
        img.img-t.img-a(src='@/assets/ilustraciones/img32.svg', alt='')
        h5.px-5.mt-5 Díptico
        p.px-5.pb-3 Lámina de papel doblada por la mitad. 

      .col-lg-4.col-color-3.white-border-1.p-0.mx-0(data-aos="fade-right")
        img.img-t.img-a(src='@/assets/ilustraciones/img33.svg', alt='')
        h5.px-5.mt-5 Tríptico
        p.px-5.pb-3  Lámina de papel doblada en tres partes. 

      .col-lg-4.col-color-3.white-border-1.p-0.mx-0(data-aos="fade-right")
        img.img-t.img-a(src='@/assets/ilustraciones/img34.svg', alt='')
        h5.px-5.mt-5 Cuadrípticos
        p.px-5.pb-3 Lámina de papel plegada en cuatro partes.                 

    .row.justify-content-center.align-items-stretch.mt-3
      .col-lg-4.col-color-3.white-border-1.p-0.mx-0(data-aos="fade-left")
        img.img-t.img-a(src='@/assets/ilustraciones/img35.svg', alt='')
        h5.px-5.mt-5 #[i Poming]
        p.px-5.pb-3 Contienen información promocional y suele entregarse en las casas, tiendas o retrovisores de los vehículos. 

      .col-lg-4.col-color-3.white-border-1.p-0.mx-0(data-aos="fade-left")
        img.img-t.img-a(src='@/assets/ilustraciones/img36.svg', alt='')
        h5.px-5.mt-5 Encartes
        p.px-5.pb-3 Folleto con varias hojas. 

      .col-lg-4.col-color-3.white-border-1.p-0.mx-0(data-aos="fade-left")
        img.img-t.img-a(src='@/assets/ilustraciones/img37.svg', alt='')
        h5.px-5.mt-5 Hojas volantes
        p.px-5.pb-3 Son folletos impresos por lado y lado; su forma de entrega es mano a mano.  

</template>

<script>
export default {
  name: 'Tema2',
  data: () => ({
    // variables de vue
  }),
  mounted() {
    this.$nextTick(() => {
      this.$aosRefresh()
    })
  },
  updated() {
    this.$aosRefresh()
  },
}
</script>

<style lang="sass" scoped></style>
